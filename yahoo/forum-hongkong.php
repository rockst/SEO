<?php
  ...
  include_once(dirname(__FILE__) . "/Yahoo_UGC_Forum.class.php");
	...
	$page_limit = (!empty($argv[2]) && intval($argv[2]) > 0) ? intval($argv[2]) : 1;
	$rows = array();
	$urls = get_url_list("http://verywed.com/forum/hongkong/list/{page}.html", "td.subject a[href$=-1.html]", $page_first, $page_limit);
  ...
  echo "Result: " . Yahoo_UGC_Forum::buildXML("tw_verywed_" . date("Ymd_His") . "_8", $rows) . "\n";
?>
