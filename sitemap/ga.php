...
EOD;
	...
	foreach($GA_input as $input) {
		...
		$funcname = "get" . $dimensions[0]; // 取得指標值的方程式
			...
			if($totalCount > 0 && count($gaResults) > 0) { // 有資料
				foreach($gaResults as $result) {
					...
					if(preg_match("/^\/[a-zA-Z0-9]{1,}/i", $uri) && !preg_match("/^\/(vwMember|mailBox|vendor\/mail|vendor\/login)+/i", $uri)) {
						...
						array_push($rows, array("URL"=>$url,"priority"=>0.8,"changefreq"=>"daily","page"=>""));
					}
				}
			} else {
				echo "-- GA 沒有任何資料\n";
	...
	$filename = "rd-ga.xml";
	if(count($rows) > 0) {
		if(buildXML($filename, $rows)) { // 產生 Sitemap 檔案
			echo "- 產生 Sitemap 完成\n";
			if(buildGZ($filename)) { // 產生 Gzip 檔案
				unlink(XMLROOT . $filename); // 刪除 XML file
				echo "- 產生 Gzip 完成\n";
				echo "FTP to beta: " . ((ftp2beta($filename . ".gz")) ? "success" : "fail") . "\n";
