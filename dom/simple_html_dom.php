...
// 取得傳入 XML 的網址
function getURL(&$rows) {
	$urls = array();
	if(gettype($rows) == "array" && count($rows) > 0) {
		foreach($rows as $item) {
			buildPageURL($urls, $item["url"], $item["pattern"], $item["page"]); // 產生分頁網址
		}
		if(gettype($urls) == "array" && count($urls) > 0) {
			fetchURL($urls); // 從 DOM 中取得連結
		}
	}
}
...
// 產生完全的網址
function makURL($url, $href) {
	// 廠商部落格特殊處理 http://verywed.com/0919111111/blog => http://verywed.com/blog/vendor.php?key=0919111111
	if(preg_match("/^http:\/\/verywed.com\/[0-9]{8,}\/blog$/i", $href)) {
		return preg_replace("/^http:\/\/verywed.com\/([0-9]{8,})\/blog$/i", "http://verywed.com/blog/vendor.php?key=\$1", $href);
	} else if(preg_match("/^http:\/\/verywed.com\/[0-9]{8,}\/blog/i", $href)) {
		return $href;
	} else if(preg_match("/^http:\/\/verywed.com/i", $href)) {
		return $href;
	} else if(preg_match("/^\//i", $href)) {
		return "http://verywed.com" . $href;
	} else {
		$parse_url = parse_url($url);
		return $parse_url['scheme'] . "://" . $parse_url['host'] . "/" . $parse_url['path'] . "/" . $href;
	}
}

/*
// 產生分頁的 URL
// 比對 $page 與網址 $url 的相關性，爾後產生多筆分頁網址
// 舉例說明：
...
*/
function buildPageURL(&$urls, $url, $pattern, $page = "") {
	if(!$page || !isset($page)) { // 沒設定 pate 則預設單筆網址
		array_push($urls, array($url, $pattern));
	} else { // 開始產生分頁
		if(preg_match_all("/[0-9]{1,}/i", $page, $match, PREG_PATTERN_ORDER) && isset($match[0][0]) && intval($match[0][0]) > 0) {
			$limit = $match[0][0];
			$regex_pattern = str_replace($limit, "[0-9]{1,}", $page);
			$p1 = "(" . substr($page, 0, strpos($page, $limit)) . ")";
			$p2 = "(" . substr($page, (strpos($page, $limit) + strlen($limit)), strlen($page)) . ")";
			for($i = 1; $i <= intval($limit); $i++) {
				array_push($urls, array(preg_replace("/\{R\}/", "", preg_replace("/" . $p1 . "[0-9]{1,}" . $p2 . "/", "\$1{R}$i{R}\$2", $url)), $pattern));
			}
		} else {
			array_push($urls, array($url, $pattern));
		}
